...
var Http = function(httpOptions = {}) {
    this.apiUrl = httpOptions.apiUrl || API_URL;
    this.credentials = {
        'access_token': httpOptions.token,
    };
};
/**
...
Http.prototype._get = function (url, parameters) {
    ...
    var getURL = this.apiUrl + '/' + url + '?' + querystring.stringify(parameters); // Construct URL with parameters
...
Http.prototype._post = function(url, parameters, body) {
    parameters = extend(parameters, this.credentials); // Add credentials to parameters
    var postURL = this.apiUrl + '/' + url + '?' + querystring.stringify(parameters); // Construct URL with parameters
...
Http.prototype._put = function(url, parameters, body) {
    parameters = extend(parameters, this.credentials); // Add credentials to parameters
    var putURL = this.apiUrl + '/' + url + '?' + querystring.stringify(parameters); // Construct URL with parameters
